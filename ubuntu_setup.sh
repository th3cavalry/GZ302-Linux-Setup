...
main() {
    ...
    if [[ "${install_secureboot,,}" == "y" || "${install_secureboot,,}" == "yes" ]]; then
        success "Secure Boot: Configured and ready (enable in BIOS)"
    fi
    
    if [[ "${install_snapshots,,}" == "y" || "${install_snapshots,,}" == "yes" ]]; then
        local fs_type=$(detect_root_filesystem)
        success "Snapshots: Configured for $fs_type (use 'gz302-snapshot' command)"
    fi
    
    success "Setup completed! Your ROG Flow Z13 (GZ302) is now optimized for Ubuntu."
    success ""
