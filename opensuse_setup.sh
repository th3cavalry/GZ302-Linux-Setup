...
# ==============================================================================
# Comprehensive OpenSUSE Setup Script for ASUS ROG Flow Z13 (2025, GZ302)
#
# Author: th3cavalry using Copilot
# Version: 1.5
#
# This script automates the post-installation setup for OpenSUSE on the
...
#
# PRE-REQUISITES:
# 1. A base installation of OpenSUSE (Tumbleweed or Leap 15.5+).
# 2. An active internet connection.
# 3. A user with sudo privileges.
#
# USAGE:
# 1. Download the script:
#    curl -O https://raw.githubusercontent.com/th3cavalry/GZ302-Arch-Setup/main/opensuse_setup.sh
# 2. Make it executable:
#    chmod +x opensuse_setup.sh
# 3. Run with sudo:
#    sudo ./opensuse_setup.sh
# ==============================================================================
...
setup_repositories() {
    ...
    local opensuse_version=$(detect_opensuse_version)
    
    # Add Packman repository for multimedia codecs
    if [[ "$opensuse_version" == "tumbleweed" ]]; then
        zypper addrepo -cfp 90 'https://ftp.gwdg.de/pub/linux/misc/packman/suse/openSUSE_Tumbleweed/' packman
    elif [[ "$opensuse_version" == "leap" ]]; then
        local leap_version=$(grep VERSION_ID /etc/os-release | cut -d'"' -f2)
        zypper addrepo -cfp 90 "https://ftp.gwdg.de/pub/linux/misc/packman/suse/openSUSE_Leap_${leap_version}/" packman
    fi
    
    # Add games repository
    if [[ "$opensuse_version" == "tumbleweed" ]]; then
        zypper addrepo 'https://download.opensuse.org/repositories/games/openSUSE_Tumbleweed/' games
    elif [[ "$opensuse_version" == "leap" ]]; then
        local leap_version=$(grep VERSION_ID /etc/os-release | cut -d'"' -f2)
        zypper addrepo "https://download.opensuse.org/repositories/games/openSUSE_Leap_${leap_version}/" games
    fi
...
install_hardware_support() {
    ...
    zypper install -y Mesa-dri Mesa-vulkan-radeon libva-utils libva2 \
        libvdpau_radeonsi1 radeontop
    
    # Install firmware and microcode
    zypper install -y kernel-firmware ucode-amd
    
    # Install power management tools
    zypper install -y powertop tlp
...
install_gaming_stack() {
    ...
    # 5a. Install multimedia codecs from Packman
    info "Installing multimedia codecs for better game compatibility..."
    ...
    # 5c. Install Lutris (from games repository)
    ...
    zypper install -y gamemode Mesa-vulkan-radeon Mesa-vulkan-radeon-32bit \
        wine winetricks
    ...
    zypper install -y MangoHud || {
        warning "MangoHUD not available in standard repos, will try Flatpak later."
    }

    # 5f. Install ProtonUp-Qt via Flatpak
    ...
    if [[ "$PRIMARY_USER" != "root" ]]; then
        ...
        # Install MangoHUD via Flatpak if not available in repos
        if ! command -v mangohud &> /dev/null; then
            sudo -u "$PRIMARY_USER" flatpak install -y flathub org.freedesktop.Platform.VulkanLayer.MangoHud
            info "MangoHUD installed via Flatpak."
        fi
    else
...
main() {
    ...
    echo
    echo "============================================================"
    echo "  ASUS ROG Flow Z13 (GZ302) OpenSUSE Setup Script"
    echo "  Version 1.3 - Gaming Performance Optimization"
    echo "  Detected: OpenSUSE $opensuse_version"
    echo "============================================================"
    echo
    
    info "Starting comprehensive setup process..."
    info "This script will configure your OpenSUSE system for optimal ROG Flow Z13 performance"
    ...
    echo
    success "============================================================"
    success "OpenSUSE setup complete for ASUS ROG Flow Z13 (GZ302)!"
    ...
    success "- Multimedia codecs from Packman repository"
    success ""
    ...
    success ""
    success "You can now enjoy OpenSUSE optimized for your"
