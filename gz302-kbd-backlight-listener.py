...
class KeyboardBacklightListener:
    ...
    def __init__(
        self, event_device: str = "/dev/input/event10", use_syslog: bool = True
    ):
        """
        ...
        try:
            with open(self.event_device, "rb") as f:
                ...
                while True:
                    ...
                    time_sec, time_usec, etype, code, value = struct.unpack(
                        "IIHHI", event_data
                    )
                    ...
                    if (
                        etype == self.EV_KEY
                        and code in self.BRIGHTNESS_KEYS
                        and value == 1
                    ):
                        action = self.BRIGHTNESS_KEYS[code]
                        self._log(
                            f"Brightness key pressed (code={code}, action={action})"
                        )
            ...
            self._log(
                "Need root access to read keyboard events. Run with sudo or as root.",
                "ERROR",
            )
            sys.exit(1)
        except FileNotFoundError:
            self._log(
                f"Input device not found: {self.event_device}. Check if device exists.",
                "ERROR",
            )
            sys.exit(1)
...
def main():
    ...
    if os.geteuid() != 0:
        print(
            "Error: This script must be run as root (use sudo or systemd service)",
            file=sys.stderr,
        )
        sys.exit(1)
