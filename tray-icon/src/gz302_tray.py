...
class GZ302TrayIcon(QSystemTrayIcon):
    ...
    def change_profile(self, profile):
        ...
        try:
            ...
            self.showMessage(
                "Error", "Command timed out", QSystemTrayIcon.MessageIcon.Critical, 5000
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if power.get("present"):
                    ...
                    if pct is not None:
                        power_line = (
                            f"Power: {pct}% {'(AC)' if plugged else '(Battery)'}"
                        )
                    else:
...
"""
            ...
            self.showMessage(
                "Autostart",
                "Autostart entry created at ~/.config/autostart/gz302-tray.desktop",
                QSystemTrayIcon.MessageIcon.Information,
                4000,
            )
        except Exception as e:
            self.showMessage(
                "Autostart Error",
                f"Failed to create autostart: {e}",
                QSystemTrayIcon.MessageIcon.Warning,
                4000,
            )
        ...
        try:
            ...
            if backlight_path.exists():
                try:
                    ...
                    if result.returncode == 0:
                        level_name = ["Off", "Level 1", "Level 2", "Level 3"][
                            brightness
                        ]
                        self.showMessage(
                            "Keyboard Backlight",
