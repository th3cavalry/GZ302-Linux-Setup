...
# ==============================================================================
# Comprehensive Pop!_OS Setup Script for ASUS ROG Flow Z13 (2025, GZ302)
#
# Author: Senior Systems Integration Engineer
# Version: 1.0
#
# This script automates the post-installation setup for Pop!_OS on the
...
#
# PRE-REQUISITES:
# 1. A base installation of Pop!_OS (22.04 LTS or newer).
# 2. An active internet connection.
# 3. A user with sudo privileges.
#
# USAGE:
# 1. Download the script:
#    curl -O https://raw.githubusercontent.com/th3cavalry/GZ302-Arch-Setup/main/popos_setup.sh
# 2. Make it executable:
#    chmod +x popos_setup.sh
# 3. Run with sudo:
#    sudo ./popos_setup.sh
# ==============================================================================
...
update_system() {
    ...
    # Upgrade system (Pop!_OS specific)
...
setup_repositories() {
    ...
    # Pop!_OS already has most gaming repos enabled, just ensure universe/multiverse
    add-apt-repository -y multiverse
    add-apt-repository -y universe
    
    # Ensure Flathub is enabled (should be by default in Pop!_OS)
...
install_hardware_support() {
    ...
    # Install AMD GPU drivers and utilities (Pop!_OS may have better defaults)
    ...
    # Install power management tools (TLP may conflict with Pop!_OS power management)
    apt install -y powertop system76-power
...
install_gaming_stack() {
    ...
    info "Pop!_OS already includes many gaming optimizations, enhancing them further..."

    # 5a. Enable 32-bit architecture for Steam (if not already enabled)
    info "Ensuring 32-bit architecture support for Steam..."
    dpkg --add-architecture i386 || true
    apt update

    # 5b. Install Steam (Pop!_OS Shop version is preferred, but fallback to apt)
    info "Installing Steam..."
    apt install -y steam-installer || {
        warning "Standard Steam package not found, trying alternative installation..."
    ...
    apt install -y lutris || {
        warning "Lutris not in standard repos, installing via Flatpak..."
    ...
    apt install -y mangohud || {
        warning "MangoHUD not in standard repos, installing via Flatpak..."
        flatpak install -y flathub org.freedesktop.Platform.VulkanLayer.MangoHud
    }

    # 5f. Install ProtonUp-Qt via Flatpak (most reliable method)
...
apply_performance_tweaks() {
    ...
    info "These optimizations will enhance Pop!_OS's existing gaming optimizations..."
...
# Gaming performance optimizations (enhanced for GZ302)
vm.swappiness = 10
vm.dirty_ratio = 15
...
# Gaming optimizations for GZ302
...
enable_services() {
    ...
    # Use System76 power management instead of TLP (they can conflict)
    info "Configuring System76 power management for gaming..."
    systemctl enable --now system76-power.service
    
    # Set performance profile
    system76-power profile performance || warning "Could not set performance profile automatically"
...
main() {
    ...
    echo "  Version 1.0 - Enhanced Gaming Performance Optimization"
    echo "============================================================"
    echo
    ...
    success "- System76 power profiles for gaming"
