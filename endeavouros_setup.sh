...
EOF
    ...
    # 4g. Enhanced Wi-Fi stability fixes (additional)
...
install_tdp_management() {
    ...
    if [[ "$PRIMARY_USER" != "root" ]] && command -v yay &> /dev/null; then
        ...
        warning "Cannot install ryzenadj without yay and non-root user."
        return 1
    fi
...
install_rocm() {
    ...
    if [[ "$PRIMARY_USER" != "root" ]] && command -v yay &> /dev/null; then
        ...
        warning "Cannot install ROCm without yay and non-root user."
        return 1
    fi
...
configure_universal_secure_boot() {
    ...
    # Sign the kernel (EndeavourOS typically uses standard kernel)
...
main() {
    ...
    info "Step 2/10: Setting up AUR helper..."
    setup_aur_helper
    ...
    echo
    success "============================================================"
    success "GZ302 EndeavourOS Setup Complete! (Version 1.5)"
